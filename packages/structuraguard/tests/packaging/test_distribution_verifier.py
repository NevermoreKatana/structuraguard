...
def test_sdist_member_limit_is_applied_during_streaming_read(tmp_path: Path) -> None:
    ...
    with tarfile.open(archive_path, mode="w:gz") as archive:
        for index in range(513):
            member = tarfile.TarInfo(f"structuraguard-0.3.0/file-{index}.txt")
            member.size = 0
...
def test_sdist_decompressed_stream_limit_covers_regular_member_payload(
    ...
    payload = b"x" * 8_192
    with tarfile.open(archive_path, mode="w:gz") as archive:
        member = tarfile.TarInfo("structuraguard-0.3.0/file.txt")
...
def test_dist_directory_rejects_gitignore(tmp_path: Path) -> None:
    (tmp_path / "structuraguard-0.3.0-py3-none-any.whl").touch()
    (tmp_path / "structuraguard-0.3.0.tar.gz").touch()
